...
export class Provider extends Component {
    ...
    getWeather(lat,lon,city) {
        const data = {};
        const arr = [];
        let newUrl = city!=null ? `${url}/weather?q=${city}&APPID=${config.key}&units=metric` : `${url}/weather?lat=${lat}&lon=${lon}&APPID=${config.key}&units=metric`;
        const { dataArray } = this.state;

        let request1 = fetch(newUrl)
            ...
            .then(json => {
                ...
                let request2 = fetch(forecastUrl)
                    .then(response => response.json()
                    .then(json => {
    ...
    render() {
        return (
            <Context.Provider value={{ state: this.state, getDay: this.getDay, getWeather:this.getWeather}}>
                {this.props.children}
            </Context.Provider>
        )
    }
